...
export default class Login extends Vue {
  account: string = '';
  password: string = '';
  accIsOK: boolean = true;
  passIsOK: boolean = true;

  closeLayer(event: any) {
  ...
  login(event: any) {
    ...
    if (!accReg.test(this.$data.account)) {
      ...
      let accountInput = <HTMLInputElement>this.$refs.account;  // 进行强制类型转换后焦点转移到这个标签上面
      accountInput.focus();
      return ;
    }
    if (!passReg.test(this.$data.password)) {
      this.$data.passIsOK = false;
      let pwdInput = <HTMLInputElement>this.$refs.password
      pwdInput.focus();
      return ;
    }
